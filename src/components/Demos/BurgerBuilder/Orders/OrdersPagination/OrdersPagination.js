...
class OrdersPagination extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    return (
      this.props.page !== nextProps.page || this.props.limit !== nextProps.limit
    );
  }
    ...
    for (let number = 1; number <= pages; number++) {
      items.push(
        <Pagination.Item
          ...
          onClick={() => this.props.goToPage(number)}
        >
          {number}
        </Pagination.Item>
      );
    ...
    return (
      <>
        <div style={pages > 1 ? { float: "right" } : { textAlign: "right" }}>
          <DropdownButton
            title={
              this.props.limit === "inf" ? (
                <FormattedMessage id="show-all" />
              ) : (
                <FormattedMessage
                  id="show-num"
                  values={{ number: this.props.limit }}
                />
              )
            }
            variant="secondary"
          >
            {this.limits.map((limit) => (
              <Dropdown.Item
                key={limit}
                onClick={() => this.props.changeLimit(limit)}
            ...
            <Dropdown.Item
              key="inf"
              onClick={() => this.props.changeLimit("inf")}
              active={0 === this.props.limit}
            >
