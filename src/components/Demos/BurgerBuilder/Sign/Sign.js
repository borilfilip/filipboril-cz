...
class Sign extends Component {
  ...
  checkFormValidity = (form) => {
    ...
    for (let inputId in form) {
      isValid =
        (form[inputId].valid ||
          (!this.isRegistering() && inputId === "passwordAgain")) &&
        isValid;
    }
    return isValid;
  };
  ...
  inputChangedHandler = (event, id) => {
    ...
    this.setState({
      ...
      formValid: this.checkFormValidity(updatedFormData),
    });
  };
  ...
  render() {
    ...
    const additionalLink = (
      <div className="mt-3">
        {register ? (
          <Link to="/demos/burger-builder/sign/in">
          ...
          <>
            ...
            <Link to="/demos/burger-builder/sign/up">
              <FormattedMessage id="do-register" />
            </Link>
          </>
        )}
      </div>
    ...
    return (
      <>
        <Notification
          title={error}
          message={<FormattedMessage id="login-failed" />}
        />
        <h2>
          {register ? (
          ...
          <form onSubmit={this.submitHandler}>
            ...
            <div className="additional-link">
              <Button
                type="submit"
                variant="primary"
                disabled={!this.state.formValid || sending}
              >
                {sending ? (
