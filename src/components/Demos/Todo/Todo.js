...
class Todo extends Component {
  ...
  deleteHandler = () => {
    axios
      .delete(this.url + "/note/" + this.state.notes[this.state.delete].id)
      .then(() => {
  ...
  saveHandler = (event, id) => {
    ...
    axios
      .put(this.url + "/note/" + item.id, { content: item.content })
      .then(() => {
        item.editing = false;
      })
      .finally(() => {
  ...
  addHandler = () => {
    ...
    axios
      .post(this.url + "/note/", { content: item.content })
      .then((response) => {
  ...
  render() {
    ...
    if (this.state.loaded)
      items = this.state.notes.map((item, idx) => {
        return (
          <Item
            key={item.id}
    ...
    let view = (
      <>
        ...
        <DeleteModal
          ...
          onClose={this.closeDeleteModalHandler}
      ...
      view = (
        <>
          ...
          <Alert variant="danger">
            <FormattedMessage id="memo-error" />
          </Alert>
        </>
      );
